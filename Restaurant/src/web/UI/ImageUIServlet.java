...
public class ImageUIServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		drawInterferes(graphics);

		String randomString = drawRandomNumber((Graphics2D) graphics);
		request.getSession().setAttribute("checkcode", randomString);
		
		response.setContentType("image/jpeg"); //the servlet returns an image
		
		//control the explorer not to use the cache
	...
	private void drawInterferes(Graphics graphics) {
	...
	private String drawRandomNumber(Graphics2D graphics) {

		graphics.setColor(Color.RED);
		graphics.setFont(new Font("ËÎÌו", Font.BOLD, 20));
		...
		for (int i = 0; i < charNumber; i++) {
			...
			graphics.rotate(degree * Math.PI / 180, XofRotate, 20);
			graphics.drawString(ch, XofRotate, YofRotate);
			graphics.rotate(-degree * Math.PI / 180, XofRotate, 20);
			XofRotate += 30;
		}
